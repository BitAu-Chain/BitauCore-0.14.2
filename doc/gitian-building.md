...
*Setup instructions for a Gitian build of Bitau Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bitau
...
- [Building Bitau Core](#building-bitau-core)
...
Clone the git repositories for bitau and Gitian.
...
in the bitau repository under 'Fetch and create inputs' to install sources which require
...
To build Bitau Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitau repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitau/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bitau-project/bitau
...
For example:
```bash
URL=https://github.com/thrasher-/bitau.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitau=${COMMIT} --url bitau=${URL} ../bitau/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitau=${COMMIT} --url bitau=${URL} ../bitau/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitau=${COMMIT} --url bitau=${URL} ../bitau/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitau git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitau/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bitau
SIGPATH=/some/root/path/bitau-detached-sigs

./bin/gbuild --url bitau=${BTCPATH},signature=${SIGPATH} ../bitau/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitau-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitau-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitau-osx-build.assert
```
...
[bitau-project/gitian.sigs.ltc](https://github.com/bitau-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
